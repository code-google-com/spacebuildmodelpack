...
ENT.MAX_JS_VAL				= 255
ENT.HALF_MAX_JS_VAL			= ENT.MAX_JS_VAL * .5
ENT.ShieldRepairTime		= 20 // in seconds
ENT.ShieldRepairMulti		= 100 / ENT.ShieldRepairTime
...
ENT.ShieldRepairStatus = self.ShieldRepairTime
...
function ENT:SpawnFunction(ply, tr)
	...
	if not ply.SBMP_JSEnts then ply.SBMP_JSEnts = {} end
...
function ENT:ProcessContraption()
	...
	if self.entities then
		for k,ent in pairs(self.entities) do
			...
			if CDS and RD_ConsumeResource then
...
function ENT:Detonate() //"Poor Johny...." -- (Guess from where this came from....)
	...
	for _,Ent in pairs(constraint.GetAllConstrainedEntities(self.Entity)) do
		if Ent != self.Entity then
			...
			elseif Ent.Entity:IsValid() then
					Ent.Entity:Remove()
			end
		end
	end
		...
		if (movetype == 2 or movetype == 3 or movetype == 5 or movetype == 6 or movetype == 8 or movetype == 9) then
			...
			elseif found:IsValid() then
				...
				elseif (string.find(found:GetClass(),"ragdoll") ~= nil) then --if it's a ragdoll...
					...
					physobj:ApplyForceOffset(vecang * (8e4 * mass), entpos + Vector(math.random(-50, 50), math.random(-50, 50), math.random(50, 50))) --still push it away
				end
...
function ENT:SoundAlarm()
	...
	if WireAddon then Wire_TriggerOutput(self.Entity, "Shield Structural Integrity", 0) end
end
...
function ENT:PhysicsSimulate(phys, deltatime)
	...
	phys:ComputeShadowControl(self.PhysSimTbl)
end
...
function ENT:CalcYaw()
	...
	return (self.MaxTurnRate * self.NavInputs[2] * -1)
end

function ENT:CalcRoll()	
	if self.NavInputs[5] then
		return (self.MaxTurnRate * self.NavInputs[2])
	end
	
	return (self.MaxTurnRate * self.NavInputs[3])
end

function ENT:OnRemove()
	if CDS and self.entities then
		for k,v in pairs(self.entities) do
			v.Shield = nil
		end
	end
	...
	if Dev_Unlink_All then Dev_Unlink_All(self.Entity) end
...
function ENT:Update(args)
	...
	elseif idx == 4 then
		self.NavInputs[idx] = val - self.MAX_JS_VAL //make it 0 based,
	elseif idx == 5 then
