	...
	if self.Pod and self.Pod:IsValid() then
		...
		if (self.CPL && self.CPL:IsValid()) then
			...
			if self.MCC then
				...
				self.CPL:PrintMessage( HUD_PRINTCENTER, "Player: " .. math.Round(PAng.z) .. ", " .. math.Round(PAng.x) )
				self.CPL:PrintMessage( HUD_PRINTCENTER, "Player: " .. math.Round(VAng.z) .. ", " .. math.Round(VAng.x) )
				
				AAng.x = VAng.x
				AAng.y = PAng.y - VAng.y
				AAng.z = PAng.z - VAng.z
				
				self.CPL:PrintMessage( HUD_PRINTCENTER, "Player: " .. math.Round(AAng.z) .. ", " .. math.Round(AAng.x) ) 
				
				CYaw = (AAng.z * math.cos(AAng.x)) - (AAng.y * math.sin(AAng.x))
				CPitch = (AAng.z * math.sin(AAng.x)) - (AAng.y * math.cos(AAng.x))
