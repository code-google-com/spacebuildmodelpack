...
function ENT:Think()
	if self.Pod and self.Pod:IsValid() then
		...
		if (self.CPL && self.CPL:IsValid()) then
			...
			if (self.CPL:KeyDown( IN_ATTACK ) || (joystick && joystick.Get(self.CPL, "ltrans_fire1"))) then
				for i = 1, self.HPC do
			...
			if (self.CPL:KeyDown( IN_ATTACK2 ) || (joystick && joystick.Get(self.CPL, "ltrans_fire2"))) then
				for i = 1, self.HPC do
