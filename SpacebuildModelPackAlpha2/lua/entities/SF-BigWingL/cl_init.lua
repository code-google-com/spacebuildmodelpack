...
function ENT:Draw()
	...
	local CAngle = self.Entity:GetFold()
	...
	local Sine = math.sin(math.rad(CAngle)) * 111
	local CoSine = math.cos(math.rad(CAngle)) * 111
	NAng:RotateAroundAxis( self.Entity:GetForward(), -CAngle )
	self.Entity:SetAngles( NAng )
	self.Entity:SetPos( SwivPos + ( self.Entity:GetRight() * CoSine ) + ( self.Entity:GetUp() * ( Sine + 15 ) ) + NAng:Up() * -18 )
