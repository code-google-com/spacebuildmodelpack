...
function ENT:Initialize()
	...
	self.Inputs = Wire_CreateInputs( self.Entity, { "Fire", "Deploy" } )
	...
	self.HP[5]["Pos"]	= Vector(16,223,14)
	self.HP[5]["Angle"]	= Angle(90,180,0)
	
	self.Cont = self.Entity
	self.Dpl = false
	self.WingAngle = 45

	self.Entity:SetFold( 45 )
...
function ENT:Think()
	...
	if self.HP[1]["Ent"] && self.HP[1]["Ent"]:IsValid() then
		local SwivPos = ( (self.Entity:GetForward() * -28) + (self.Entity:GetRight() * -223) + (self.Entity:GetUp() * 18))
		local NAng = self.Entity:GetAngles()
		local Sine = math.sin(math.rad( self.WingAngle )) * 222
		local CoSine = math.cos(math.rad( self.WingAngle )) * 222
		NAng:RotateAroundAxis( self.Entity:GetForward(), -self.WingAngle )
		NAng:RotateAroundAxis( NAng:Up(), 180 )
		self.HP[1]["Ent"]:SetLocalAngles( self.Entity:WorldToLocalAngles(NAng) )
		self.HP[1]["Ent"]:SetLocalPos( SwivPos + ( self.Entity:GetRight() * CoSine ) + ( self.Entity:GetUp() * ( Sine + 15 ) ) )
	end
