			...
			if (joystick) then
				...
				if (joystick.Get(self.CPL, "sbepftr_accelerate")) then
					if (joystick.Get(self.CPL, "sbepftr_accelerate") > 128) then
						self.Speed = math.Clamp(self.Speed + (joystick.Get(self.CPL, "sbepftr_accelerate")/127.5-1)*self.AccelMax, -self.MinSpeed, self.MaxSpeed)
					elseif (joystick.Get(self.CPL, "sbepftr_accelerate") < 127) then
						self.Speed = math.Clamp(self.Speed + (joystick.Get(self.CPL, "sbepftr_accelerate")/127.5-1)*self.DecelMax, -self.MinSpeed, self.MaxSpeed)
					end
				end
				...
				if (joystick.Get(self.CPL, "sbepftr_thrust")) then
					if (joystick.Get(self.CPL, "sbepftr_thrust") > 128) then
						self.TarSpeed = (joystick.Get(self.CPL, "sbepftr_thrust")/127.5-1)*self.MaxSpeed
					elseif (joystick.Get(self.CPL, "sbepftr_thrust") < 127) then
						self.TarSpeed = (joystick.Get(self.CPL, "sbepftr_thrust")/127.5-1)*self.MinSpeed
				...
				if (joystick.Get(self.CPL, "sbepftr_pitch")) then
					if (joystick.Get(self.CPL, "sbepftr_pitch") > 128) then
						self.Pitch = -(joystick.Get(self.CPL, "sbepftr_pitch")/127.5-1)*self.TSpeed
