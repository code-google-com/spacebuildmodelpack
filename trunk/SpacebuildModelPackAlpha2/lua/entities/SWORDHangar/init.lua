...
--magic function ent:GetVar("Cont").Launchy
function ENT:Touch( ent )
	if ( ent:IsValid() && ent:IsVehicle() && (ent:GetModel()=="models/slyfo/sword.mdl") && (ent.Cont != nil) && !ent.Cont.Launchy ) then
		if ( (self.SWORD1 == nil) && (ent != self.SWORD2) ) then
			self.SWORD1 = ent
			self.SWORD1:SetPos( self.Entity:LocalToWorld(Vector(-50, 400, -100)) )
			self.SWORD1:SetAngles( self.Entity:GetAngles() )
		...
		elseif ( (self.SWORD2 == nil) && (ent != self.SWORD1) ) then
			self.SWORD2 = ent
			self.SWORD2:SetPos( self.Entity:LocalToWorld(Vector(-50, -400, -100)) )
			self.SWORD2:SetAngles( self.Entity:GetAngles() )
